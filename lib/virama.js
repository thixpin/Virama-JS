...
class MyVirama {
  ...
  constructor(testerID) {
    ...
    if(document.getElementById(this.testerID)){ 
			this.browserFont = this.detectBrowserFont();
		}  else {
			this.init();
    }
  ...
  async init(){
	...
	detectBrowserFont(){
		...
		const browserFont = (one_ka * 1.5 > two_ka ) ? 'Unicode' : 'Zawgyi';
  ...
  getBrowserFont(){
		...
		if(document.getElementById(this.testerID) || this.addTestDivToBody() ) {
			this.browserFont = this.detectBrowserFont();
		} 
  ...
  replace_with_rule(rule, output){
	...
	isMyanmarText(output) {
		const MyanmarReg = new RegExp("[\u1000-\u1021]");
		return MyanmarReg.test(output) ? true : false;
	}


  uni2zg(output){

		if(!this.isMyanmarText(output)) {
			return output;
		}
		...
		return this.replace_with_rule(rule, output);
	}
  ...
  isUnicodeUser(){
		return (this.getBrowserFont() == 'Unicode') ;
  }
    

  write(output){
		return this.isUnicodeUser() ? this.uniNormalize(output) : this.zg2uni(output);
	}
    
    
	read(output){
		return this.isUnicodeUser() ? output : this.uni2zg(output);
  }
